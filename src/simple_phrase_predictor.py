#!/usr/bin/env python3
"""
Simple Trump Phrase Mention Probability Predictor

A lightweight version using only Python standard library.
"""
...
def bayesian_beta_model(historical_pct: float, days_elapsed: int, days_in_month: int, 
    ...
    """
    Bayesian model using Beta distribution approximation.
    """
...
def poisson_time_decay_model(historical_pct: float, days_elapsed: int, 
    ...
    initial_daily_rate = -math.log(1 - historical_pct) / days_in_month
    ...
    prob_at_least_one = 1 - math.exp(-total_remaining_intensity)
...
def logistic_time_model(historical_pct: float, days_elapsed: int, 
    ...
    base_logit = math.log(historical_pct / (1 - historical_pct + 1e-10))
    time_adjustment = -steepness * progress
    adjusted_logit = base_logit + time_adjustment
    probability = 1 / (1 + math.exp(-adjusted_logit))
