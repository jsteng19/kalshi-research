...
def _expand_hyphenated_word(word: str) -> List[str]:
    """
    Expand a hyphenated word into its three forms: hyphenated, spaced, and closed compound.
    
    Args:
        word: A word that may contain hyphens (e.g., "Alley-oop", "All-Star")
    
    Returns:
        List of three forms: [hyphenated, spaced, closed]
        If word has no hyphens, returns [word] (single item)
    """
    if '-' not in word:
        return [word]
    
    # Generate the three forms
    hyphenated = word  # Original form
    spaced = word.replace('-', ' ')  # Space form
    closed = word.replace('-', '')  # Closed compound form
    
    return [hyphenated, spaced, closed]


def _get_word_forms_list(word: str) -> List[str]:
    """
    Get all forms (singular, plural, possessive) for a single word as a list.
    Also expands hyphenated words into hyphen, space, and closed compound forms.
    ...
    """
    # First, expand hyphenated words (e.g., "Alley-oop" -> ["Alley-oop", "Alley oop", "Alleyoop"])
    hyphen_expansions = _expand_hyphenated_word(word)
    
    # For each expansion, generate singular/plural/possessive forms
    all_forms = []
    
    for expanded_word in hyphen_expansions:
        ...
        clean_word = expanded_word.lower().strip('".,!?;:')
        ...
        # Generate forms for this expansion
        forms = []
