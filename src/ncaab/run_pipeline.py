#!/usr/bin/env python3
"""
Full NCAAB mentions pipeline: update data, find open events, match crews,
generate reports, and open in browser.

Usage:
    python src/ncaab/run_pipeline.py
    python src/ncaab/run_pipeline.py --skip-update        # skip data refresh
    python src/ncaab/run_pipeline.py --skip-update --skip-series  # use cached series results
"""
...
from src.ncaab.generate_report import (
    load_announcer_data,
    lookup_crew_for_event,
    build_game_report,
    render_html_report,
)
from src.ncaab.ncaab_config import get_announcer_crew, find_crew_by_announcer_name

SERIES = "KXNCAABMENTION"
COMBINED_CSV = "data/ncaab/combined_announcers.csv"
TRANSCRIPT_DIR = "data/ncaab/transcripts"
...
def main():
    parser = argparse.ArgumentParser(description="Full NCAAB mentions pipeline")
    parser.add_argument("--skip-update", action="store_true",
                        help="Skip data update (506 scrape + merge)")
    ...
    if not args.skip_update:
        step("1/5  Updating game data (506 Sports)")
        from src.ncaab.update_game_data import update_506_data, merge_all_sources
        try:
            update_506_data()
