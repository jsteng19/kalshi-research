...
class YouTubeTranscriptDownloader:
    def __init__(self, use_proxy=True, max_retries=5):
        ...
        # Rate limiting - increased defaults to avoid 429 errors
        self.min_delay = 8.0
        self.max_delay = 15.0
    ...
    def download_transcript(self, video_id):
        """Download transcript using the configured API with retry logic for rate limits."""
        for retry in range(self.max_retries):
            try:
                # Try different language preferences
                # for lang_codes in [['en'], ['en-US'], ['en-GB'], ['English - DTVCC1'], None]:  # None = auto-detect
                for lang_codes in [['en'], None]:  # None = auto-detect
                    try:
                        if lang_codes:
                        ...
                        if '429' in error_str or 'too many' in error_str or 'rate limit' in error_str:
                            ...
                            logger.warning(f"Rate limited on {video_id}, retry {retry + 1}/{self.max_retries}. Waiting {backoff_time:.1f}s")
                            time.sleep(backoff_time)
                            break  # Break inner loop to retry with backoff
                        
                        # Handle language-related errors
                        if 'transcript' in error_str or 'language' in error_str:
                            continue  # Try next language
                        else:
                            raise e  # Re-raise non-language related errors
                else:
                    # If we exhausted all language options without a rate limit, break retry loop
                    break
        ...
        logger.error(f"Failed to download transcript for {video_id} after {self.max_retries} retries")
        return None
...
def main():
    ...
    parser.add_argument('--delay-min', type=float, default=8.0, help='Minimum delay between requests (seconds)')
    parser.add_argument('--delay-max', type=float, default=15.0, help='Maximum delay between requests (seconds)')
    parser.add_argument('--max-retries', type=int, default=5, help='Maximum retries for rate-limited requests')
    ...
    downloader = YouTubeTranscriptDownloader(use_proxy, max_retries=args.max_retries)
